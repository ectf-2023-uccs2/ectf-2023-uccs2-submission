...
int sha256_test()
{
    ...
    BYTE text1[] = PASSWORD;
    BYTE text2[] = CAR_ID;
    size_t byte_len = sizeof(text4) + sizeof(text1) + sizeof(text2);
    BYTE text3[byte_len];
    size_t offset = 0;
    memcpy(text3 + offset, text4, sizeof(text4));
    offset += sizeof(text4);
...
//this will decrypt the received buffer and get the hash
#ifdef EXAMPLE_AES
 ...
 // AES_init_ctx(&ctx, key);
  ...
  // If the data transfer is the same hash as the fob then unlock
